...
import gwybridge
from gwybridge._run import ENV_VAR, find_converter
...
def test_converter_not_found(no_converter_anywhere):
    with pytest.raises(gwybridge.ConverterNotFoundError):
        find_converter()


def test_env_var_pointing_nowhere(monkeypatch):
    monkeypatch.setenv(ENV_VAR, "/no/such/gwyconvert")
    with pytest.raises(gwybridge.ConverterNotFoundError, match=ENV_VAR):
        find_converter()
