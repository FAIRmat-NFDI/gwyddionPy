...
```
gwyddionpy.load(raw) → dict → pynxtools-spm reader adapter → NXspm .nxs → NOMAD
```

That adapter (a reader that accepts "whatever gwyddionpy emits") instantly
gives NOMAD ingestion of every Gwyddion-supported format. Keep the adapter in
the pynxtools-spm codebase, not here — gwyddionpy stays schema-agnostic.

## Supporting a new raw file format

Don't add parsers here. New formats belong upstream in Gwyddion
(`gwyddion/modules/file/*.c`) — then gwyddionpy inherits them automatically
